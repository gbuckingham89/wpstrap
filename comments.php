...
<div class="comments-form">
	...
	$args = array( 
		'fields' => apply_filters( 
			...
			array(
			    'author' => '<label for="author">Name' . ( $req ? ' *' : '' ) . '</label><input class="span4" id="author" name="author" type="text" value="' . esc_attr( $commenter['comment_author'] ) . '" size="30" tabindex="1"' . $aria_req . ' />',
			    'email'  => '<label for="email">Email' . ( $req ? ' *' : '' ) . '</label><input class="span4" id="email" name="email" type="text" value="' . esc_attr(  $commenter['comment_author_email'] ) . '" size="30" tabindex="2"' . $aria_req . ' />',
	    ...
	    'must_log_in' => '<p class="must-log-in">' .  sprintf( __( 'You must be <a href="%s" title="Login">logged in</a> to post a comment.' ), wp_login_url( apply_filters( 'the_permalink', get_permalink( $post_id ) ) ) ) . '</p>',
